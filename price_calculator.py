...
def final_price(price, quantity, discount_pct, tax_pct):
    ...
    sub_2=apply_discount(subtotal, discount_pct)
    final_prc=apply_tax(sub_2,tax_pct)
    return round(final_prc,2)
...
def best_deal(price_a, qty_a, disc_a, price_b, qty_b, disc_b, tax_pct):
    ...
    """
    price_A = final_price(price_a, qty_a, disc_a, tax_pct)
    price_B =final_price(price_b, qty_b, disc_b, tax_pct)
    if price_A > price_B:
        return "B"
